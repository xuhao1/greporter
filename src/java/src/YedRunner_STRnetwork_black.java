...
public class YedRunner_STRnetwork_black
{
	
	public YedRunner_STRnetwork_black(File[] inputfiles, File outputdirectory) throws Exception
	{
		for(int x = 0; x < inputfiles.length; x++)
		{
			//generate the graphs
			System.out.println(inputfiles[x]);
			YedGraph_STRnetwork_black temp = new YedGraph_STRnetwork_black(inputfiles[x]);
	...
	public void save(YedGraph_STRnetwork_black graph, File file, int index)
	{
		switch(index)
		{
