...
def Ann_Limma(doc, w, tag, agd, adgr):
    ...
    for gene in genes:
        ...
        if change.find('+')!=-1:
            rowsup.append(countrow)
        writeline = gene+'\t'+judgeValue(geneinfo[2])+'\t'+desc
        tablecontent = tablecontent+writeline+'\n'
        countrow  += 1
...
def geneSummery(doc, w, tag, labels, adgr1, sdagr1, adgr2, sdagr2, stolr):

    rng = rmtag(doc, w, tag)
    tablecontent = '\t'.join(['Gene', 'Probability, %s' % labels.typemap['tp1'],
                              'Probability, %s' % labels.typemap['tp2']]) + '\n'
                              # header
                              # 3 columns
    rowsup1 = []
    rowsup2 = []
    ...
    for gene in genelist:
        ...
        writeline = '\t'.join([gene, judgeValue(prob1), judgeValue(prob2)])
        tablecontent = tablecontent + writeline + '\n'
        countrow += 1
    ...
    for row in range(2, countrow):
        ...
        if row in rowsoverlap:
            ...
            tb.Cell(row,3).Range.Font.Color = 0x9900CC    #purple
